...
class L2Loss(EmbeddingLoss):
    ...
    def forward(self, preds, target):
        ...
        return torch.norm((preds - target_embedded.transpose(1, 2)), p=2, dim=1).masked_select(mask).mean()
...
class CosineLoss(EmbeddingLoss):
    ...
    def forward(self, preds, target):
        target_embedded = self.tgt_embedding(target)
        mask = target.ne(self.pad_id)
        return 1 - cosine_similarity(preds.transpose(1, 2), target_embedded, dim=2, eps=1e-8).masked_select(mask).mean()
